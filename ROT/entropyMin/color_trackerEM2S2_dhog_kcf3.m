function [positions, fps] = color_trackerEM2S2_dhog_kcf3(params)
...
fre = 1;                             % update frequency 更新频率
fre2 = 1;                            % update frequency of high variation 突变阈值 更新频率
Thr_Occ = 1;                         % threshold of occlusion 遮挡阈值
Thr_Occ_learning_rate = 0.015;       %0.075;
% trans_vec = [-1,0;1,0;0,-1;0,1];     %4 direction 4个方向  
...
im_sz = size(im);
paddingH = struct('generic', 1.0, 'large', 1, 'height', 0.4);
sz = get_search_window(target_sz, im_sz, paddingH);
% sz = floor(target_sz * (1 + padding));

% discriminate occlusion 用于判断遮挡
cell_size = 8;%floor(min(target_sz)/4);%param.cell_size; %size of hog feature, hog特征的范围大小
...
%             d_next = sqrt(sqdist(tpatch,new_tpatch))/length(new_tpatch);
        ...
        %% train current classifier 训练当前帧的分类器
        %%%%%%%%%%%%%%%%% update CN model  %%%%%%%%%%%%%%%%%
        [xo_npca, xo_pca] = get_subwindow_s(im, pos, sz_s, sz,non_compressed_features, compressed_features, w2c);
...
%             n_class = 1; n_con = 0;
            ...
            if n_con == fre-1% update 达到更新频率开始更新
            ...
            if n_con2 == fre2-1 
                % select threshold of high variation and occlusion 选择突变与遮挡的阈值
                n_trans = size(trans_vec8,1);
                com_patch = zeros(dp,np,n_trans);
                count = 0;
                for j = 1:n_trans
...
%                         end
        ...
        % update threshold of high variation 更新突变阈值
        Thr_next = 2*d_next;
    end     
